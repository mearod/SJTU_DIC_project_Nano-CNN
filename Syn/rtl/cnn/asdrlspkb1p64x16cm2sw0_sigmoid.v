...
module asdrlspkb1p64x16cm2sw0_sigmoid ( Q, ADR, D, WE, ME, CLK, TEST1, RM, RME);
...
generic_behav_asdrlspkb1p64x16cm2sw0_sigmoid #( MES_ALL) uut ( .Q(Q_tmp), .ADR(ADR), .D(D), .WE(WE), .ME(ME), .CLK(CLK) );
...
module generic_behav_asdrlspkb1p64x16cm2sw0_sigmoid (  Q, ADR, D, WE, ME, CLK );

parameter MES_ALL = "ON";
parameter words = 256, bits = 32, addrbits = 8;
...
wire [1:0] ADR_valid;
reg [31:0] mem_core_array [0:256-1];
